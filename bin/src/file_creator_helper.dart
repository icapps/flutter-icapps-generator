...
class FileCreatorHelper {
  ...
  static List<String> getSortedImports(String file, List<String> extraImports) {
    final imports = <String>[];
    const LineSplitter().convert(file).forEach(
      (l) {
        if (l.startsWith('import ')) imports.add(l);
      },
    );
    imports
      ..addAll(extraImports)
  ...
  static void createScreenFile(String projectName, String screenName, {required bool generateDI}) {
    final imports = [
      ...
      "import 'package:$projectName/navigator/route_names.dart';",
  ...
  static Future<void> updateMainNavigator(String projectName, String screenName) async {
    ...
    final fileString = await mainNavigatorFile.readAsString();
    sb
      ..writeAll(
        getSortedImports(
          fileString,
      ...
      ..writeln('');
    const LineSplitter().convert(fileString).forEach(
      (l) {
  ...
  static Future<void> updateRouteNames(String screenName) async {
    ...
    if (!routeNameFile.existsSync()) {
      print('`lib/navigator/route_names.dart` does not exists. Can not add navigation logic.');
      return;
    }
